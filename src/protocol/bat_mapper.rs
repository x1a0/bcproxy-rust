...
impl BatMapper {
    pub fn new(mut input: BytesMut) -> BatMapper {
                ...
                b';' if input.len() - 1 > i && input[i + 1] == b';' => {
                    ...
                    match state {
                        ...
                        ParseState::FromDirection => {
                            from = Some(latin1_to_string(&bytes));
                            output.extend(&[b'\n'][..]);
...
mod tests {
    ...
    fn area_room() {
        ...
        assert_eq!(bat_mapper.from, Some(String::from("east")));
